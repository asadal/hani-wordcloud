...
# 3단계 데이터 입력 선택
st.header("1. 데이터 입력")
input_method = st.radio("데이터 입력 방법을 선택하세요:",
                        ("텍스트 파일 업로드", "엑셀 파일 업로드", "직접 입력"))
...
if input_method == "텍스트 파일 업로드":
    ...
    if txt_file is not None:
        ...
        tokens = [
            word for word, pos in words_with_pos if pos in desired_pos and word not in stopwords]
    ...
    xlsx_file = st.file_uploader("엑셀 파일을 업로드하세요(.xlsx). 칼럼은 '단어'와 '빈도수'로 지정해주세요.", type=["xlsx"])
    if xlsx_file is not None:
        ...
        if '단어' in df.columns and '빈도수' in df.columns:
            df = df[['단어', '빈도수']].dropna()
            df = df.sort_values(
                by='빈도수', ascending=False).reset_index(drop=True)
...
with col1:
    max_words = st.slider("최대 단어 수", min_value=20,
                          max_value=200, value=100, step=10)

with col2:
    theme = st.selectbox("테마 선택 (Colormap)", options=[
